...
def _get_call(url):
    return requests.get(url, headers=HEADERS, auth=AUTH, verify=False)


def _post_call(url, body):
    return requests.post(url, data=json.dumps(body), headers=HEADERS, auth=AUTH, verify=False)


def _put_call(url, body):
    return requests.put(url, data=json.dumps(body), headers=HEADERS, auth=AUTH, verify=False)


def _delete(url):
    r = requests.delete(url, headers={'X-Allow-Overwrite': 'true'}, auth=AUTH, verify=False)
...
def main():
    get_existing_pools()
    lb_rules = create_lb_pool_and_rule(56)
    # print lb_rules
